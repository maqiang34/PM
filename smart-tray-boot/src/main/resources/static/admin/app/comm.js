if(!window.sessionStorage.asikey){
    // parent.sysuser=undefined;window.sessionStorage.clear();
    // window.top.location="/";
}else{
...
function initalertTable(title,iconCls,method,url,queryParams,toptol,fottol,col,isautosize,onDblClickRow) {
    if (isautosize) {
        tablesize = stablesize = parseInt((($("#showGood").height() - 80) / 26));
    }
    if (tablesize < 10) {
        tablesize = stablesize = 10;
    }
     $('#showGood').datagrid({
        url: url,
        title: title,
    ...
    if (fottol) {
        $('#showGood').datagrid().datagrid('getPager').pagination(fottol);	// get the pager of datagrid
    }
}
/*
* 扩展校验规则
