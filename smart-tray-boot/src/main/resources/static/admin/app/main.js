...
function createFrame(url) {
    ...
    return '<iframe scrolling="no"  marginheight="0" marginwidth="0" frameborder="0"  src="view/' + url + '" style="width:100%;height:100%;"></iframe>';
}
    ...
    $.each(usermenus, function (j, o) {
        ...
        if (o.child && o.child.length > 0) {
            $.each(o.child, function (k, p) {
                var badge='';
                if(p.menuid=='0_4'){//认证
                    badge='<span id="authObadge" class="fr oBadge"></span>';
                }else if(p.menuid=='0_9'){//意见反馈
                    badge='<span id="problemObadge" class="fr oBadge"></span>';
                }else if(p.menuid=='0_11'){//合伙人申请
                    badge='<span id="partnerObadge" class="fr oBadge"></span>';
                }else if(p.menuid=='0_14'){//一键出租求租
                    badge='<span id="demandObadge" class="fr oBadge"></span>';
                }
                menulist += '<li><div><a ref="' + p.menuid + '" href="#" rel="' + p.url + '" ><span class="icon ' + p.icon + '" > </span><span class="nav">' + p.menuname + '</span>'+badge+'</a></div> </li>'
            });
        }
        menulist += '</ul>';
...
function changCount(count, name) {
    if(count==0){
        $("#"+name).hide();
    }else {
        $("#"+name).html(count).show();
    }
}
...
function checkuser() {
    $.ajax({
        type: "GET", cache: false, dataType: 'json', url: "/i/admin/getUser", success: function (data) {
...
$().ready(function () {
    if (!window.sessionStorage.sysadmin || !window.sessionStorage.asikey) {
        top.location.href = "http://" + location.host ; return;
    }
